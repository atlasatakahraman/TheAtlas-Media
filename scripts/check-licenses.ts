 ...
 *
 * Policy (00-Meta/Hard-Rules.md in the vault):
 *   ALLOW   MIT, Apache-2.0, BSD-*, ISC, MPL-*, Unlicense, CC0, Zlib,
 *           GPL-3.0 (any form), GPL-2.0-or-later
 *   DENY    AGPL-*, SSPL-*, and any GPL not allowed above (GPL-2.0-only, GPL-1.0)
 *   REVIEW  anything else, including missing
 *
 * Judged against shipping under GPL-3.0-only. GPL-3.0 code combines with it;
 * AGPL would force the whole work to AGPL, SSPL is not open source, and
 * GPL-2.0-only cannot be combined with GPL-3.0.
 *
...
const DENY = /^(GPL|AGPL|SSPL)/i;   // reached only by a GPL that ALLOW did not take
const ALLOW = /^(MIT.*|Apache-2\.0.*|BSD.*|ISC|MPL-2\.0|MPL|Unlicense|CC0-1\.0|0BSD|Zlib|Python-2\.0|Artistic-2\.0|Unicode-3\.0|Unicode-DFS-.*|CDLA-Permissive-.*|CC-BY-[0-9.]+|BlueOak-.*|GPL-3\.0.*|GPL-2\.0(\+|-or-later))$/i;
...
function ffmpegNote(): string[] {
  ...
  notes.push(
    ...
    (bundled
      ? `A bundled ffmpeg artefact was found under \`packaging/\` — its own terms travel with it, including the source offer a GPL build requires.`
...
if (denied.length) {
  console.log("\nDENIED — incompatible with GPL-3.0-only:");
